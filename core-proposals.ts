...
export class CoreProposalQueue {
  ...
  enqueue(draft: ProposalDraft): CoreMemoryProposal {
    ...
    const dupe = this.proposals.find(
      (p) => p.status === "pending" && p.scope.sessionKey === draft.scope.sessionKey && p.key === candidateKey && p.value === candidateValue,
    );
    if (dupe) return dupe;
...
export function extractCoreProposal(text: string, scope: MemoryScope): ProposalDraft | null {
  ...
  const patterns: Array<{ rx: RegExp; keyPrefix: string; reason: string }> = [
    { rx: /^my name is (.+)$/i, keyPrefix: "profile.name", reason: "user identity preference" },
    { rx: /^i (?:prefer|like) (.+)$/i, keyPrefix: "preference", reason: "user preference statement" },
    { rx: /^i(?:'m| am) from (.+)$/i, keyPrefix: "profile.location", reason: "user profile statement" },
    { rx: /^(?:please )?remember (?:that )?(.+)$/i, keyPrefix: "remember", reason: "explicit remember request" },
    { rx: /^we (?:use|are using) (.+)$/i, keyPrefix: "team.stack", reason: "team/project persistent detail" },
  ];
  ...
  for (const p of patterns) {
    ...
    return { text: msg, key, value, reason: p.reason, scope };
  }
