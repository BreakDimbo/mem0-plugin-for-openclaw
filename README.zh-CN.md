...
A: 这是 mem0 上游 Graph Memory 的兼容性问题，不只是缺少 `baseURL`。当前插件在 `enableGraph=true` 且 LLM provider 为 `google/gemini` 时，会自动把 Graph LLM 转成 Google 的 OpenAI-compatible 配置，并同步镜像到 mem0 误读的顶层 `llm.config`。通常不需要再手动写 `graph_store.llm`。如果你要显式覆盖，也应使用 OpenAI-compatible 形态：
