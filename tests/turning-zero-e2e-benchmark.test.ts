...
assert(CASES.length === 40, `expected 40 cases, got ${CASES.length}`);
assert(new Set(CASES.map((item) => item.id)).size === 40, "case ids must be unique");
assert(CASES.every((item) => item.query.trim().length > 0), "queries must be non-empty");
assert(CASES.every((item) => item.expected.trim().length > 0), "expected values must be non-empty");

console.log("turning-zero-e2e-benchmark.test.ts ok");
