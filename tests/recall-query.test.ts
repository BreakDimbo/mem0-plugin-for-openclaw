...
await test("createRecallHook loads session core cache once and reranks against current query", async () => {
  ...
  const hook = createRecallHook(
    ...
    { getBySender: async () => "" } as any,
    {
      ...
      core: { enabled: true, topK: 5, maxItemChars: 240, autoExtractProposals: false, humanReviewRequired: false, touchOnRecall: false, proposalQueueMax: 10, alwaysInjectTiers: ["profile", "general"], retrievalOnlyTiers: ["technical"], alwaysInjectLimit: 600 },
      ...
      sync: { enabled: false, intervalMs: 300, memoryFilePath: "MEMORY.md" },
    } as any,
    { info: () => {}, warn: () => {} },
...
await test("createRecallHook reuses relevant selections for similar session queries", async () => {
  ...
  const hook = createRecallHook(
    ...
    { getBySender: async () => "" } as any,
    {
      ...
      core: { enabled: false, topK: 5, maxItemChars: 240, autoExtractProposals: false, humanReviewRequired: false, touchOnRecall: false, proposalQueueMax: 10, alwaysInjectTiers: ["profile", "general"], retrievalOnlyTiers: ["technical"], alwaysInjectLimit: 600 },
      backend: { freeText: { provider: "mem0" } },
      mem0: { mode: "open-source", enableGraph: false, searchThreshold: 0.3, topK: 5 },
      capture: { enabled: false, maxItemsPerRun: 0, minChars: 0, maxChars: 0, maxConversationTurns: 1, dedupeThreshold: 0.8 },
      outbox: { enabled: false, concurrency: 1, batchSize: 1, maxRetries: 1, drainTimeoutMs: 1000, persistPath: "", flushIntervalMs: 1000 },
  ...
  const hook = createRecallHook(
    ...
    { getBySender: async () => "" } as any,
    {
      ...
      recall: {
        ...
        topK: 2, threshold: 0.3, maxChars: 1200,
